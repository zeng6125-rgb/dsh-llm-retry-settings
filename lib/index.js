// D:/dsh/fixbug/deepseek-harness/vendor/cosmokit/src/misc.ts
...
// D:/dsh/fixbug/deepseek-harness/vendor/cosmokit/src/types.ts
...
// D:/dsh/fixbug/deepseek-harness/vendor/cosmokit/src/time.ts
var Time;
((Time2) => {
  Time2.millisecond = 1;
  Time2.second = 1e3;
...
// D:/dsh/fixbug/deepseek-harness/vendor/schemastery/lib/index.mjs
...
// src/index.ts
var name = "@dsh-external/dsh-llm-retry-settings";
var inject = ["settings"];
var NS = "dsh-llm-retry";
var Config = Schema.object({
  enabled: Schema.boolean().default(false),
  maxRetries: Schema.number().step(1).min(0).default(2),
  initialDelayMs: Schema.number().min(1).default(500),
  maxDelayMs: Schema.number().min(1).default(1e4),
  jitterRatio: Schema.number().min(0).max(1).default(0.1),
  // 内置默认补充码：400 reasoning_text（INVALID_REQUEST）与 pi-ai 兜底错误
  // （PI_AI_ERROR，覆盖 STREAM_ERROR 等流式失败）。开启 enabled 即自动拦截重试，
  // 用户可在设置卡片追加/清空。
  retryableCodes: Schema.array(Schema.string()).default(["INVALID_REQUEST", "PI_AI_ERROR"])
});
function resolveConfig(config) {
  ...
  return {
    ...
    maxDelayMs: Math.max(1, Math.floor(config?.maxDelayMs ?? 1e4)),
...
function apply(ctx, config) {
  ...
  const syncFromScope = (next) => {
    ...
    if (Array.isArray(next.retryableCodes)) {
      live.retryableCodes = next.retryableCodes.filter((c) => typeof c === "string" && c.length > 0);
    }
  ...
  ctx.on(
    ...
    (payload, next) => {
      if (live.enabled && payload && payload.retryPolicy && typeof payload.retryPolicy === "object") {
        ...
        const mergedCodes = live.retryableCodes.length > 0 ? [.../* @__PURE__ */ new Set([...p.retryableCodes ?? [], ...live.retryableCodes])] : p.retryableCodes;
        payload.retryPolicy = {
          ...p,
