...
// [2026-08-20 compat] rc.8 移除了 @deepseek-ai/dsh-client-web-react 静态模块；
// bindSnapshotSelector 是 uSES selector bridge，用 react.useSyncExternalStore 内联等价实现。
var import_dsh_client_web_react = { bindSnapshotSelector: (w) => {
  var subscribe = (fn) => w.subscribe(fn);
  var getSnapshot = () => w.getSnapshot();
  return function useSelector(sel) {
    return sel(import_react.useSyncExternalStore(subscribe, getSnapshot));
  };
} };
